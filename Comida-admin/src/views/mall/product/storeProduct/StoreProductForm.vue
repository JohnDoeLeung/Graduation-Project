      ...
      <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
        ...
        <el-tab-pane label="基本信息" name="one">
          <el-form-item label="展示店铺" prop="shopId">
            <el-select v-model="formValidate.shopId" placeholder="选择店铺" @change="selectShop">
              <el-option v-for="item in shopList" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
          
          <el-form-item label="菜品名称" prop="store_name">
            <el-input v-model="formValidate.store_name" class="input-width" placeholder="请输入菜品名称" />
          </el-form-item>
          
          <el-form-item label="菜品分类" prop="cate_id">
            <el-select v-model="formValidate.cate_id" placeholder="选择店铺" @change="selectShop">
          ...
          <el-form-item label="菜品状态" prop="is_show">
            <el-radio-group v-model="formValidate.is_show">
              <el-radio :label="1">上架</el-radio>
              <el-radio :label="0">下架</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="菜品简介" prop="store_info">
            <el-input type="textarea" rows="5" v-model="formValidate.store_info" placeholder="请输入菜品简介" />
          </el-form-item>
        </el-tab-pane>

        <el-tab-pane label="菜品规格" name="two">
          <el-form-item label="菜品规格" prop="spec_type">
          ...
          <el-col :span="24" v-if="formValidate.spec_type === 1" class="noForm">
              ...
              <el-form-item label="选择规格：" prop="">
                <div class="acea-row row-middle">
                  <el-select v-model="formValidate.selectRule">
                    <el-option v-for="(item, index) in ruleList" :value="item.ruleName" :key="index">{{ item.ruleName
                      }}</el-option>
                  </el-select>
                ...
                <div v-for="(item, index) in attrs" :key="index" style="width: 100%">
                  ...
                  <div class="rulesBox">
                    <el-tag type="dot" closable color="primary" v-for="(j, indexn) in item.detail" :key="indexn"
                      :name="j" class="mr20" @close="handleRemove2(item.detail, indexn)">{{ j }}</el-tag>
                    <el-input placeholder="请输入属性名称" v-model="item.detail.attrsVal" style="width: 150px">
                      <template #append>
            ...
            <el-col :xl="24" :lg="24" :md="24" :sm="24" :xs="24" v-if="manyFormValidate.length">
              <!-- 多规格表格-->
              <el-col :span="24">
                <el-form-item label="菜品属性：" class="labeltop">
                    ...
                    <el-table-column type="myindex" v-for="(item, index) in formValidate.header" :key="index"
                      ...
                      <template #default="scope">
                        <div v-if="item.slot == 'pic'" align="center">
                          <Materials v-model="scope.row[scope.column.property]" num="1" type="image" :width="60"
                            :height="60" />
        ...
        <el-tab-pane label="营销设置" name="three">
...
const formRules = reactive({
  ...
  image: [{ required: true, message: '菜品图片不能为空', trigger: 'blur' }],
  slider_image: [{ required: true, message: '轮播图不能为空', trigger: 'blur' }],
  store_name: [{ required: true, message: '菜品名称不能为空', trigger: 'blur' }],
...
const getInfo = (id) => {
  // let that = this;
  StoreProductApi.getStoreProductInfo(id)
    .then(async (res) => {
...
// 立即生成
const generate = () => {
  StoreProductApi.isFormatAttr(formValidate.value.id, { attrs: attrs.value })
    .then((res) => {

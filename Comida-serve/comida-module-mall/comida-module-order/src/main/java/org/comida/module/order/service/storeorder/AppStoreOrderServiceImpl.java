...
import org.comida.module.order.controller.app.order.param.AppOrderParam;
import org.comida.module.order.controller.app.order.param.AppPayParam;
...
import org.comida.module.order.dal.mysql.ordernumber.OrderNumberMapper;
import org.comida.module.order.dal.mysql.storeorder.StoreOrderMapper;
import org.comida.module.order.enums.AppFromEnum;
import org.comida.module.order.enums.OrderLogEnum;
import org.comida.module.order.enums.OrderStatusEnum;
import org.comida.module.order.enums.PayTypeEnum;
import org.comida.module.order.service.storeorder.dto.StatusDto;
import org.comida.module.order.service.storeordercartinfo.StoreOrderCartInfoService;
...
import static org.comida.module.member.enums.ErrorCodeConstants.COUPON_NOT_CONDITION;
import static org.comida.module.member.enums.ErrorCodeConstants.USER_ADDRESS_NOT_EXISTS;
...
public class AppStoreOrderServiceImpl extends ServiceImpl<StoreOrderMapper,StoreOrderDO> implements AppStoreOrderService {
    ...
    public AppStoreOrderQueryVo getOrderInfo(String unique, Long uid) {
        ...
        wrapper.and(
                i -> i.eq(StoreOrderDO::getOrderId, unique).or().or());
        if (uid != null) {
    ...
    public Map<String, Object> createOrder(Long uid,  AppOrderParam param) {
        ...
        if(OrderLogEnum.ORDER_TAKE_OUT.getValue().equals(param.getOrderType())){
            payPrice = NumberUtil.sub(NumberUtil.add(sumPrice,postagePrice),couponPrice);
        }else{
            payPrice = NumberUtil.sub(sumPrice,couponPrice);
        }
       // BigDecimal payPrice = NumberUtil.sub(NumberUtil.add(sumPrice,postagePrice),couponPrice);
        ...
        switch (PayTypeEnum.toType(param.getPaytype())){
            case WEIXIN:
                if(AppFromEnum.H5.getValue().equals(param.getFrom())){
                    detailsId = "4";
                    //todo 如果启用微信H5支付充值 需要另外增加一个配置用于同步跳转页面 比如下面的增加了一个id=5的配置
//                    if(orderInfo != null) {
//                        detailsId = "4";
//                    }else{
//                        detailsId = "5";
//                    }
                    ...
                    throw exception(new ErrorCode(99999,"不支持公众号支付"));

                } else {
                    MerchantPayOrder payOrder = new MerchantPayOrder("3", "JSAPI", msg,
                            msg, price, param.getUni());
    ...
    public AppStoreOrderQueryVo handleOrder(AppStoreOrderQueryVo order) {
        ...
        if (OrderStatusEnum.STATUS_0.getValue().equals(order.getPaid())) {
            ...
            if(OrderLogEnum.ORDER_TAKE_OUT.getValue().equals(order.getOrderType())){
                statusDTO.set_title("配送中");
            }else{
                statusDTO.set_title("待取餐");
            }
            ...
            if(OrderLogEnum.ORDER_TAKE_OUT.getValue().equals(order.getOrderType())){
                statusDTO.set_title("已收货");
            }else{
                statusDTO.set_title("已取餐");
            }
